...
export default function SearchCoreClient() {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case "service":
        return <Wrench className="w-4 h-4 text-[#B87333] shrink-0 mt-0.5" />;
      case "area":
        return <MapPin className="w-4 h-4 text-emerald-655 shrink-0 mt-0.5" />;
      case "project":
        return <Briefcase className="w-4 h-4 text-[#0F2137] shrink-0 mt-0.5" />;
      case "blog":
    ...
    <div className="flex flex-col gap-8">
      ...
      <form onSubmit={handleSearchSubmit} className="relative w-full">
        ...
        <input
          ...
          className="w-full bg-white border border-slate-250 focus:border-[#B87333] px-12 py-4 rounded-2xl text-sm sm:text-base focus:outline-hidden focus:ring-2 focus:ring-[#B87333]/10 shadow-xs"
        />
        <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-500 pointer-events-none" />
        <button
          type="submit"
          className="absolute right-3 top-1/2 -translate-y-1/2 bg-[#0F2137] hover:bg-[#B87333] text-white text-xs font-bold px-4.5 py-2.5 rounded-xl cursor-pointer transition-colors shadow-sm"
        >
          Search Site
        </button>
      </form>
          ...
          {results.length === 0 ? (
            <div className="bg-white rounded-3xl p-10 border border-slate-200 text-center flex flex-col items-center gap-3">
              ...
              <p className="text-slate-500 text-xs leading-relaxed max-w-sm">
                Try searching for general keywords like &ldquo;balcony&rdquo;, &ldquo;invisible&rdquo;, &ldquo;Adyar&rdquo;, or &ldquo;price&rdquo; to find listings.
              </p>
            </div>
          ) : (
            <div className="flex flex-col gap-4">
              {results.map((match, idx) => (
                <Link
                  key={idx}
                  href={match.url}
                  className="bg-white rounded-2xl p-5 border border-slate-200 hover:border-[#B87333]/30 shadow-xs hover:shadow-md transition-all flex gap-4 group text-left"
                >
                  {getIcon(match.type)}
                  ...
                  <div className="flex items-center self-center shrink-0 text-slate-500 group-hover:text-[#B87333] group-hover:translate-x-0.5 transition-all">
