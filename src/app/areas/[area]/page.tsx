...
export async function generateMetadata({ params }: PageProps) {
  ...
  if (!areaDetail) {
    return {
      title: "Area Not Found | Ram Enterprises - Safety Nets & Invisible Grills Chennai, Tamil Nadu",
    };
  }

  const title = `Safety Nets & Invisible Grills in ${areaDetail.name} | Ram Enterprises Chennai, Tamil Nadu`;
  const description = `Ram Enterprises provides professional balcony safety nets, pigeon netting, and SS316 invisible grills in ${areaDetail.name}, Chennai, Tamil Nadu. Free doorstep inspection across ${areaDetail.localities.slice(0, 5).join(", ")}. Call +91 90305 05683.`;
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `https://ramenterpriseschennai.com/areas/${resolvedParams.area}`,
      images: [
        {
          url: "/images/og-image.webp",
          width: 1200,
          height: 630,
          alt: `Safety Nets in ${areaDetail.name} - Ram Enterprises`,
        },
      ],
      type: "website",
    },
...
export default async function AreaDetailPage({ params }: PageProps) {
  ...
  const localFAQs = [
    {
      ...
      answer: `No. Ram Enterprises offers 100% free site visits, catalog displays, and measurements in ${areaDetail.name} and surrounding Chennai, Tamil Nadu locations without any payment commitments.`,
    },
    {
    ...
    {
      question: `Can Ram Enterprises invisible grills handle the weather in ${areaDetail.name}?`,
      answer: `Yes. Our invisible grills utilize marine-grade SS316 stainless steel wires wrapped in a clear nylon protective casing. They are highly rustproof and resist Chennai, Tamil Nadu's extreme heat and coastal sea moisture.`,
    },
    {
  ...
  const whatsappMessage = encodeURIComponent(
    `Hello Ram Enterprises! I would like to request a free site inspection for safety nets / invisible grills in ${areaDetail.name}.`
  );
    ...
    <div className="bg-slate-50 min-h-screen py-8 md:py-12">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav className="flex items-center gap-2 text-xs font-semibold text-slate-500 mb-6" aria-label="Breadcrumb">
          ...
          <span>/</span>
          <Link href="/areas" className="hover:text-[#B87333]">
            Areas
          </Link>
          <span>/</span>
        ...
        {/* Hero Banner */}
        <div className="relative rounded-3xl overflow-hidden bg-gradient-to-br from-[#06101C] via-[#0F2137] to-[#1A3A5C] text-white p-8 sm:p-12 mb-12 shadow-xl">
          <div className="max-w-3xl space-y-5">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-white/10 backdrop-blur-md rounded-full text-xs font-bold text-amber-200 border border-white/15">
              <MapPin className="w-3.5 h-3.5 text-[#B87333]" />
              <span>Doorstep Service across {areaDetail.name}, Chennai</span>
            </div>
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight leading-tight">
              Safety Nets &amp; Invisible Grills in <span className="text-[#B87333]">{areaDetail.name}</span>
            </h1>
            ...
            <div className="flex flex-wrap gap-3 pt-2">
              <a
                ...
                className="flex items-center justify-center gap-2 bg-[#B87333] text-white font-extrabold px-6 py-3 rounded-full hover:bg-amber-200 text-xs shadow-lg transition-transform hover:scale-105"
              >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                {allServices.map((srv) => (
                  <Link
                    ...
                    className="p-3.5 bg-slate-50 hover:bg-[#FDF6F0]/70 border border-slate-200/80 rounded-2xl flex flex-col justify-between group transition-all"
                  >
                    <div>
                      <span className="text-[10px] font-bold uppercase tracking-wider text-[#B87333] block mb-1">
                        {srv.categoryName}
                      </span>
                      <h4 className="text-xs font-bold text-slate-800 group-hover:text-[#B87333] transition-colors line-clamp-2">
                        {srv.name} in {areaDetail.name}
                      </h4>
                    </div>
                    <div className="pt-3 mt-3 border-t border-slate-200/60 flex items-center justify-between text-[11px] font-bold text-[#B87333]">
              ...
              <div className="space-y-2">
                {nearbyAreas.map((city) => (
                  <Link
                    ...
                    className="flex items-center justify-between p-3 bg-slate-50 hover:bg-[#FDF6F0] rounded-xl border border-slate-200 text-xs font-bold text-slate-700 hover:text-[#B87333] transition-colors"
                  >
                    <span>{city.name}</span>
