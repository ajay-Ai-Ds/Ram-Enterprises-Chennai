...
export async function generateMetadata({ params }: PageProps) {
  ...
  if (!post) {
    return {
      title: "Article Not Found | Ram Enterprises - Safety Nets & Invisible Grills Chennai, Tamil Nadu",
    };
  }

  const title = `${post.title} | Ram Enterprises - Safety Nets & Invisible Grills Chennai, Tamil Nadu`;
  const description = `${post.summary} - Expert insights by Ram Enterprises across Chennai.`;
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `https://ramenterpriseschennai.com/blog/${resolvedParams.slug}`,
      images: [
        {
          url: post.image,
          width: 1200,
          height: 630,
          alt: `Ram Enterprises - ${post.title}`,
        },
      ],
      type: "article",
    },
    ...
    <div className="bg-slate-50 min-h-screen py-12">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav className="flex items-center gap-2 text-xs font-semibold text-slate-500 mb-8" aria-label="Breadcrumb">
          ...
          <span>/</span>
          <Link href="/blog" className="hover:text-[#B87333]">
            Blog
          </Link>
          <span>/</span>
        ...
        <div className="max-w-4xl mb-10 text-left flex flex-col gap-4">
          <span className="bg-[#FDF6F0] text-[#B87333] text-[10px] font-bold uppercase tracking-wider px-3.5 py-1.5 rounded-full inline-block self-start border border-orange-100">
            {post.categoryLabel}
          </span>
          ...
          <div className="lg:col-span-8 flex flex-col gap-10 text-left">
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-10 border border-slate-200 shadow-xs flex flex-col gap-8">
              ...
              <div className="bg-slate-50 rounded-2xl p-5 border border-slate-200 flex flex-col gap-3">
                ...
                <ul className="space-y-2 text-xs sm:text-sm text-[#0F2137] font-bold">
                  {post.contentBlocks.map((block: any, i: number) => {
                    const anchorId = `section-${i}`;
                    return (
                      <li key={i} className="hover:text-[#B87333] transition-colors">
                        <a href={`#${anchorId}`}>
                          {i + 1}. {block.heading}
                        </a>
                      </li>
                  ...
                  <li className="hover:text-[#B87333] transition-colors">
              ...
              {/* Introduction */}
              <p className="text-slate-500 text-sm sm:text-base leading-relaxed border-l-4 border-[#B87333] pl-4 italic">
                {post.introduction}
              </p>
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-xs flex flex-col gap-6 scroll-mt-24" id="faq-section">
              <h3 className="text-base font-bold text-slate-800 border-b border-slate-100 pb-3 flex items-center gap-2">
                <HelpCircle className="w-5 h-5 text-[#B87333] shrink-0" />
            ...
            <div className="grid grid-cols-2 gap-4 border-t border-slate-200 pt-8">
              {prevPost ? (
                <Link
                  ...
                  className="flex items-center gap-3 p-4 bg-white hover:bg-[#FDF6F0] border border-slate-200 rounded-2xl hover:border-[#B87333]/20 text-left group"
                >
                  <ArrowLeft className="w-4 h-4 text-slate-500 group-hover:-translate-x-1 transition-transform" />
                  <div className="flex flex-col gap-0.5 truncate">
                    <span className="text-[10px] uppercase font-bold text-slate-500">Previous</span>
                    <span className="text-xs font-bold text-slate-700 group-hover:text-[#B87333] transition-colors truncate">
                      {prevPost.title}
                    </span>
                  </div>
                </Link>
              ...
              {nextPost ? (
                <Link
                  ...
                  className="flex items-center justify-between p-4 bg-white hover:bg-[#FDF6F0] border border-slate-200 rounded-2xl hover:border-[#B87333]/20 text-right group"
                >
                  <div className="flex flex-col gap-0.5 truncate text-right w-full">
                    ...
                    <span className="text-xs font-bold text-slate-700 group-hover:text-[#B87333] transition-colors truncate block">
                      {nextPost.title}
                    </span>
                  </div>
              ...
              <div className="flex flex-col gap-2.5">
                {relatedServices.map((service) => (
                  <Link
                    ...
                    className="flex items-center justify-between p-3.5 bg-slate-50 hover:bg-[#FDF6F0]/50 border border-slate-100 rounded-xl transition-all group"
                  >
                    <span className="text-xs font-bold text-slate-700 group-hover:text-[#B87333] transition-colors truncate">
                      {service.name}
                    </span>
                    <ArrowRight className="w-3.5 h-3.5 text-slate-500 group-hover:translate-x-0.5 group-hover:text-[#B87333] transition-all" />
                  </Link>
                ))}
              </div>
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {relatedAreas.map((area) => (
                  <Link
                    ...
                    className="text-[11px] font-semibold px-2.5 py-2 bg-slate-50 hover:bg-[#FDF6F0] border border-slate-200 rounded-lg text-slate-600 hover:text-[#B87333] hover:border-[#B87333]/30 transition-all truncate"
                  >
                    {area.name}
                  </Link>
                ))}
              ...
              <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-xs flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-3">
                  {relatedBlogs.map((blog) => (
                    ...
                    >
                      <span className="text-xs font-bold text-slate-850 group-hover:text-[#B87333] transition-colors leading-snug line-clamp-2">
                        {blog.title}
                      </span>
