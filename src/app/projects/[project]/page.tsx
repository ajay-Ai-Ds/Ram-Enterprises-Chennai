...
export async function generateMetadata({ params }: PageProps) {
  ...
  if (!project) {
    return {
      title: "Project Not Found | Ram Enterprises - Safety Nets & Invisible Grills Chennai, Tamil Nadu",
    };
  }

  const title = `${project.name} | Ram Enterprises - Safety Nets & Invisible Grills Chennai, Tamil Nadu`;
  const description = `${project.description} Project executed by Ram Enterprises in ${project.location}, Chennai, Tamil Nadu.`;
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `https://ramenterpriseschennai.com/projects/${resolvedParams.project}`,
      images: [
        {
          url: project.image,
          width: 1200,
          height: 630,
          alt: `Ram Enterprises - ${project.name}`,
        },
      ],
    },
    twitter: {
    ...
    <div className="bg-slate-50 min-h-screen py-12">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav className="flex items-center gap-2 text-xs font-semibold text-slate-500 mb-8" aria-label="Breadcrumb">
          ...
          <span>/</span>
          <Link href="/projects" className="hover:text-[#B87333]">
            Projects
          </Link>
          <span>/</span>
        ...
        <div className="flex flex-col gap-4 mb-10 text-left">
          <div className="flex flex-wrap items-center gap-3 text-xs text-slate-500 font-bold uppercase">
            <span className="bg-[#B87333] text-white text-[10px] px-3 py-1 rounded-full">
              {project.serviceName}
            </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 mb-16">
          ...
          <div className="lg:col-span-5 flex flex-col gap-6">
            <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-xs flex flex-col gap-6">
              ...
              <div className="pt-2">
                <a
                  href="tel:+91 90305 05683"
                  className="w-full flex items-center justify-center gap-2.5 bg-[#B87333] hover:bg-accent-hover text-white font-bold py-3.5 rounded-xl transition-colors text-xs shadow-xs"
                >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
          ...
          <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 flex flex-col gap-4">
            <h3 className="text-base font-bold text-slate-800 flex items-center gap-2">
              <ShieldAlert className="w-5 h-5 text-[#B87333] shrink-0" />
          ...
          <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 flex flex-col gap-4">
            <h3 className="text-base font-bold text-slate-800 flex items-center gap-2">
              ...
              <span>Ram Enterprises Solution</span>
            </h3>
            <p className="text-slate-500 text-xs sm:text-sm leading-relaxed">
              {project.solution}
            </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 mb-16 items-start">
          ...
          <div className="lg:col-span-5 bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 flex flex-col gap-5">
            <h3 className="text-base font-bold text-slate-800 flex items-center gap-2">
              <Wrench className="w-4.5 h-4.5 text-[#B87333]" />
            ...
            <ul className="space-y-3.5 text-xs sm:text-sm text-slate-600">
              {project.materialsUsed.map((mat, i) => (
                <li key={i} className="flex items-center gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-[#B87333] shrink-0"></div>
                  <span>{mat}</span>
                </li>
              ))}
            </ul>
        ...
        <div className="bg-[#0F2137] text-white rounded-3xl p-8 border border-primary-800 text-center flex flex-col items-center gap-3 mb-20 max-w-2xl mx-auto shadow-md">
          <h4 className="text-sm font-bold text-white uppercase tracking-wider">Project Verified Status</h4>
          <span className="text-xs font-extrabold text-[#B87333] uppercase bg-white/10 border border-white/20 px-3 py-1 rounded-md">
            100% Quality Inspected &amp; Warranty Approved
          </span>
          <p className="text-xs text-slate-300 leading-relaxed">
            This installation passed all tension safety tests, anchor load checks, and high-rise structural compliance standards across Chennai.
          </p>
        </div>
              ...
              {relatedProjects.map((item) => (
                ...
                >
                  ...
                  <div className="p-5 flex flex-col flex-grow">
                    <h3 className="text-base font-bold text-slate-800 mb-1 group-hover:text-[#B87333] transition-colors">
                    ...
                    <Link
                      ...
                      className="flex items-center justify-between text-xs font-bold uppercase tracking-wider text-[#0F2137] hover:text-[#B87333] transition-colors group/btn pt-3 border-t border-slate-50 mt-auto"
                    >
                      <span>Explore</span>
