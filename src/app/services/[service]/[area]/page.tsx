...
export async function generateMetadata({ params }: PageProps) {
  ...
  if (!serviceDetail || !areaDetail) {
    return {
      title: "Service Location Not Found | Ram Enterprises",
    };
  }

  const title = `${serviceDetail.name} in ${areaDetail.name} | Free Inspection & Same-Day Service - Ram Enterprises`;
  const description = `Get expert ${serviceDetail.name} installation in ${areaDetail.name}, Chennai, Tamil Nadu. UV-treated virgin HDPE mesh, SS316 marine-grade invisible grills, 5-year warranty, and free doorstep measurement across ${areaDetail.localities.slice(0, 5).join(", ")}. Call +91 90305 05683.`;
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `https://ramenterpriseschennai.com/services/${service}/${area}`,
      images: [
        {
          url: serviceDetail.image || "/images/og-image.webp",
          width: 1200,
          height: 630,
          alt: `${serviceDetail.name} in ${areaDetail.name} - Ram Enterprises`,
        },
      ],
      type: "website",
    },
...
export default async function ServiceLocationPage({ params }: PageProps) {
  ...
  const localFAQs = [
    ...
    },
    {
      ...
      answer: `Yes, every ${serviceDetail.name} installation by Ram Enterprises in ${areaDetail.name} includes an official 3 to 5 Year Manufacturer & Workmanship Warranty Card.`,
    },
    {
      question: `Is your bird netting and spike installation 100% humane and safe?`,
      answer: `Yes, absolutely. Ram Enterprises strictly adheres to non-lethal, eco-friendly bird exclusion methods complying with Indian animal welfare regulations. Our nets and blunt spikes create a harmless physical barrier with zero harm to pigeons or other wildlife.`,
    },
    {
  ...
  // Same service in other Chennai, Tamil Nadu cities
  const sameServiceInOtherAreas = areaDetail.nearbySlugs
    .map((slug) => areasData[slug])
    .filter(Boolean);

  const whatsappMessage = encodeURIComponent(
    `Hello Ram Enterprises! I am interested in ${serviceDetail.name} installation in ${areaDetail.name}. Please share a free quote.`
  );
  ...
  return (
    <div className="bg-slate-50 min-h-screen">
      ...
      {/* Hero Section */}
      <div className="relative bg-gradient-to-b from-[#06182C] via-[#0F2137] to-[#0D3B66] text-white pt-8 pb-16 lg:pt-12 lg:pb-24 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <nav className="flex flex-wrap items-center gap-2 text-xs font-semibold text-amber-200/70 mb-6" aria-label="Breadcrumb">
            ...
            <span>/</span>
            <span className="text-[#B87333] font-bold">{areaDetail.name}</span>
          </nav>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-12 items-center">
            ...
            <div className="lg:col-span-7 space-y-6">
              {/* Trust Badge */}
              <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white/10 backdrop-blur-md border border-white/15 text-xs font-semibold text-amber-200">
              ...
              <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black text-white tracking-tight leading-[1.15]">
                Professional <span className="text-[#B87333]">{serviceDetail.name}</span> in {areaDetail.name}
              </h1>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-2 gap-3 pt-2">
                ...
                <div className="flex items-center gap-2.5 p-3 rounded-2xl bg-white/5 border border-white/10">
                  <Award className="w-5 h-5 text-[#B87333] shrink-0" />
              ...
              <div className="flex flex-wrap items-center gap-3 pt-4">
                <a
                  ...
                  className="flex items-center justify-center gap-2 px-6 py-3.5 bg-gradient-to-r from-[#B87333] to-[#01579B] hover:from-[#039BE5] hover:to-[#B87333] text-white font-extrabold text-sm rounded-2xl shadow-xl shadow-sky-950/40 hover:scale-[1.02] transition-transform"
                >
                  <Phone className="w-4 h-4 animate-bounce" />
                  <span>Call Now: +91 90305 05683</span>
                </a>
                <a
      ...
      <section className="py-12 bg-white border-b border-slate-200/80">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-8">
            <span className="text-xs font-bold uppercase tracking-widest text-[#B87333] bg-[#FDF6F0] px-3 py-1 rounded-full inline-block mb-2">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-2.5">
            {areaDetail.localities.map((locality, idx) => (
              <div
                key={idx}
                className="flex items-center gap-2 p-3 bg-slate-50 hover:bg-[#FDF6F0]/70 border border-slate-200/70 rounded-xl transition-colors text-slate-800"
              >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
            ...
            <div className="lg:col-span-6 space-y-6">
              <span className="text-xs font-bold uppercase tracking-widest text-[#B87333] bg-emerald-50/60 px-3 py-1 rounded-full inline-block">
                Built for {areaDetail.name} High-Rises
              </span>
              <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">
                Why {areaDetail.name} Homeowners Trust Ram Enterprises
              </h2>
            ...
            <div className="lg:col-span-6 bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm">
              ...
              <div className="divide-y divide-slate-100 text-xs">
                ...
                <div className="py-2.5 flex justify-between items-center gap-4">
                  ...
                  <span className="text-[#B87333] font-bold text-right">{areaDetail.name}, Chennai, Tamil Nadu</span>
                </div>
                <div className="py-2.5 flex justify-between items-center gap-4">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="bg-slate-50 rounded-3xl p-6 sm:p-8 border border-slate-200 relative">
              <span className="w-10 h-10 rounded-2xl bg-[#B87333] text-white font-extrabold flex items-center justify-center text-base mb-4 shadow-md shadow-sky-800/20">
                1
              </span>
        ...
        <section className="py-16 bg-slate-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center max-w-2xl mx-auto mb-10">
              <span className="text-xs font-bold uppercase tracking-widest text-[#B87333] bg-emerald-50/60 px-3 py-1 rounded-full inline-block mb-2">
              ...
              {areaDetail.customerReviews.map((rev, idx) => (
                ...
                >
                  ...
                  <div className="pt-4 border-t border-slate-100 mt-4 flex items-center justify-between">
                    ...
                    <span className="text-[10px] font-bold text-[#B87333] bg-[#FDF6F0] px-2 py-0.5 rounded-md">
                      Verified
                    </span>
                  </div>
                </div>
              ...
              >
                <summary className="flex items-center justify-between cursor-pointer font-bold text-sm text-slate-900">
                  <span className="flex items-center gap-2.5">
                    <HelpCircle className="w-4 h-4 text-[#B87333] shrink-0" />
                    {faq.question}
                  </span>
                  <span className="text-xs font-bold text-[#B87333] group-open:rotate-180 transition-transform">
                    ▼
                  </span>
                </summary>
      ...
      <section className="py-14 bg-slate-100/70 border-t border-slate-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {otherServicesInArea.map((s) => (
                  <Link
                    ...
                    className="p-3 bg-white hover:bg-[#FDF6F0] rounded-xl border border-slate-200 text-xs font-bold text-slate-700 hover:text-[#B87333] flex items-center justify-between transition-colors shadow-2xs"
                  >
                    <span>{s.name}</span>
            ...
            {/* Same service in nearby Chennai, Tamil Nadu cities */}
            <div>
              <h3 className="text-sm font-extrabold uppercase tracking-wider text-slate-800 mb-4 flex items-center gap-2">
                <MapPin className="w-4 h-4 text-[#B87333]" />
                {serviceDetail.name} in Other Chennai, Tamil Nadu Cities
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-slate-900/95 backdrop-blur-md border-t border-slate-800 p-3 flex items-center gap-2 shadow-2xl">
        <a
          href="tel:+919030505683"
          className="flex-1 py-3 px-3 bg-[#B87333] text-white text-xs font-extrabold rounded-xl flex items-center justify-center gap-1.5 shadow-md active:scale-95 transition-transform"
        >
          <Phone className="w-4 h-4" />
          <span>Call: 9030505683</span>
        </a>
        <a
          href={`https://wa.me/919030505683?text=${whatsappMessage}`}
          target="_blank"
