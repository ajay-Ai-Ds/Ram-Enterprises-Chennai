...
export async function generateMetadata({ params }: PageProps) {
  ...
  if (!serviceDetail) {
    return {
      title: "Service Not Found | Ram Enterprises - Safety Nets & Invisible Grills Chennai, Tamil Nadu",
    };
  }

  const title = `${serviceDetail.name} | Ram Enterprises - Safety Nets & Invisible Grills Chennai, Tamil Nadu`;
  const description = `${serviceDetail.description} Professional installation by Ram Enterprises in Chennai across Adyar, Velachery, Anna Nagar, T. Nagar, Tambaram, Porur, and OMR Sholinganallur.`;
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `https://ramenterpriseschennai.com/services/${resolvedParams.service}`,
      images: [
        {
          url: serviceDetail.image,
          width: 1200,
          height: 630,
          alt: `Ram Enterprises - ${serviceDetail.name}`,
        },
      ],
    },
    twitter: {
    ...
    <div className="bg-slate-50 min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <nav className="flex items-center gap-2 text-xs font-semibold text-slate-500 mb-6" aria-label="Breadcrumb">
          ...
          <span>/</span>
          <Link href={`/services/${serviceDetail.category}`} className="hover:text-[#B87333] capitalize">
            {serviceDetail.categoryName}
          </Link>
          <span>/</span>
      ...
      {/* Why Choose Ram Enterprises Section */}
      <ServiceWhyChoose />
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-2xl mx-auto mb-16 flex flex-col gap-3">
            ...
            <p className="text-slate-500 text-xs sm:text-sm">
              Drag the central divider bar to see the direct aesthetic difference between unprotected spaces and a secure Ram Enterprises setup.
            </p>
          </div>
          <BeforeAfterSlider
