            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-4">
              <div className="flex flex-col gap-3 bg-slate-50 p-5 rounded-2xl border border-slate-100">
                <div className="flex items-center gap-2 font-bold text-slate-800 text-sm">
                  <ShieldAlert className="w-4.5 h-4.5 text-[#B87333]" />
              ...
              <div className="flex flex-col gap-3 bg-slate-50 p-5 rounded-2xl border border-slate-100">
                <div className="flex items-center gap-2 font-bold text-slate-800 text-sm">
                  <Settings className="w-4.5 h-4.5 text-[#B87333]" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-auto md:h-[600px]">
          ...
          <div className="md:col-span-1 relative h-80 md:h-full rounded-3xl overflow-hidden border border-slate-200 shadow-md bg-slate-200">
            <Image
              src={imagesList[0]}
              alt={`Ram Enterprises - ${name} main installation view in Chennai`}
              fill
          ...
          <div className="md:col-span-2 grid grid-cols-2 gap-6 h-full">
            {imagesList.slice(1, 5).map((imgUrl, idx) => (
              ...
              >
                <Image
                  src={imgUrl}
                  alt={`Ram Enterprises - ${name} detail snapshot ${idx + 1}`}
                  fill
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {benefits.map((benefit, index) => (
            <motion.div
              ...
              className="bg-slate-50 p-6 rounded-2xl border border-slate-200/60 flex items-start gap-4 hover:shadow-md hover:border-[#B87333]/20 transition-all duration-300 group"
            >
              <div className="p-2.5 bg-white border border-slate-200 text-[#B87333] rounded-xl shrink-0 group-hover:bg-[#B87333] group-hover:text-white transition-colors duration-300">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-3xl overflow-hidden shadow-md border border-slate-200">
          <table className="w-full text-left border-collapse text-xs sm:text-sm">
            <thead>
              <tr className="bg-[#0F2137] text-white font-bold">
