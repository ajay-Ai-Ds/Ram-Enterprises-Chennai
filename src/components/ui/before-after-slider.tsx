...
export default function BeforeAfterSlider({
  ...
  afterLabel = "After Ram Enterprises Setup",
  className = "",
  ...
  return (
    <div
      ...
      tabIndex={0}
      className={`relative w-full h-[400px] sm:h-[480px] rounded-3xl overflow-hidden shadow-lg border border-slate-200 select-none cursor-ew-resize focus:outline-hidden focus:ring-2 focus:ring-[#B87333] ${className}`}
    ...
    >
      ...
      <div className="absolute inset-0 w-full h-full bg-slate-200">
        <Image
          src={beforeImage}
          alt={`Ram Enterprises - ${beforeLabel}`}
          fill
      ...
      >
        <Image
          src={afterImage}
          alt={`Ram Enterprises - ${afterLabel}`}
          fill
          sizes="(max-width: 1024px) 100vw, 50vw"
      ...
      >
        {/* Drag Circle Handle */}
        <div className="w-10 h-10 rounded-full bg-[#B87333] text-white border-2 border-white flex items-center justify-center shadow-lg transform -translate-x-1/2 cursor-ew-resize hover:scale-105 active:scale-95 transition-transform duration-200">
